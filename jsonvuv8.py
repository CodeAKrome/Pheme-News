...
class JsonViewer:
    def __init__(self, stdscr, filename: str):
        ...
        self.current_export_index = 0  # Index in sorted selected_ids for export mode
        self.load_data()
        self.load_summary()
        self.setup_curses()
    ...
    def draw_export(self):
        ...
        for idx, field in enumerate(fields[start:end], start):
            ...
            self.stdscr.addnstr(row, 0, item_str, max_x - 1, attrs)
            row += 1
        self.stdscr.addstr(max_y - 1, 0, "Up/Down: Select Field | Space: Toggle | Enter: Export | Esc: Cancel | Right/Left: Next/Prev Record")
    ...
    def handle_export_input(self, key):
        ...
        if key == curses.KEY_UP:
            ...
            self.cursor_pos = min(len(fields) - 1, self.cursor_pos + 1)
        elif key == curses.KEY_LEFT and self.current_export_index > 0:
            ...
            self.cursor_pos = 0
        elif key == curses.KEY_RIGHT and self.current_export_index < len(self.selected_ids) - 1:
            self.current_export_index += 1
            selected_list = sorted(self.selected_ids)
            self.current_export_record = next((item for item in self.data if item['id'] == selected_list[self.current_export_index]), {})
            self.cursor_pos = 0
