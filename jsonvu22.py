            ...
            if value and isinstance(value[0], dict):
                ...
                for item in value:
                    for k, v in item.items():
                        line = f"    {k}: {v}"
                        if any(
                            term.lower() in line.lower() for term in highlight_terms
                        ):
                            line = "  " + line
                        lines.append(line)
                lines.append("]")
    ...
    def get_displayed_records():
        ...
        for r in candidates:
            ...
            if filter_ner_tag:
                ner_filtered = any(
                    any(
                        span["value"] == filter_ner_tag
                        for span in sent.get("spans", [])
                    )
...
def show_entity_popup():
    """Show a scrollable popup listing all unique NER span.text values."""
    if not span_list:
        return None
    ...
    choice = 0
    scroll_pos = 0
    visible_items = popup_height - 2  # Exclude borders

    while True:
        popup_win.clear()
        popup_win.border()
        popup_win.addstr(
            0, (popup_width - len(title)) // 2, title, curses.color_pair(5)
        )
        ...
        if key == curses.KEY_UP:
            ...
            new_choice = choice - visible_items
            if new_choice >= 0:
                choice = new_choice
            else:
                choice = 0
                scroll_pos = max(scroll_pos - visible_items, 0)

        elif key == curses.KEY_NPAGE:  # Page Down
            new_choice = choice + visible_items
            if new_choice < len(span_list):
                if new_choice < scroll_pos + visible_items:
                    choice = new_choice
                else:
                    scroll_pos = min(
                        scroll_pos + visible_items, len(span_list) - visible_items
                    )
                    choice = scroll_pos + visible_items - 1
            else:
                scroll_pos = min(
                    scroll_pos + visible_items, len(span_list) - visible_items
                )
    ...
    def show_entity_popup000():
        ...
        while True:
            ...
            if key == curses.KEY_UP:
                ...
                if (
                    choice < visible_items - 1
                    and scroll_pos + choice < len(span_list) - 1
                ):
                    choice += 1
                elif scroll_pos + visible_items < len(span_list):
                    scroll_pos += 1
    ...
    def update_display():
        ...
        if not displayed_records:
            ...
            lines = format_record(
                record, max_x - 4, [search_query] if search_query else []
            )
            ...
            for i in range(display_height):
                ...
                if any(
                    ...
                    for term in ([search_query] if search_query else [])
                ):
    ...
    def update_status(message=None):
        ...
        parts.append(
            "↑↓: Sources | ←→: Navigate | PageUp/Down: Scroll | F: Toggle NER Filter | E: Entities | Esc: Clear Filters | q: Quit"
        )
        text = "".join(parts)
    ...
    def update_search():
        search_win.clear()
        search_win.addstr(
            0, 0, "Search: " + search_query[: max_x - 8], curses.color_pair(2)
        )
    ...
    while True:
        ...
        if key == ord("q"):
            ...
            if sources:
                ...
                is_searching = True
                search_results = [
                    records[i] for i in source_to_records[selected_source]
                ]
                current_index = 0
                scroll_offset = 0
                update_display()
            ...
            if displayed_records:
                scroll_offset = min(
                    ...
                    max(
                        0,
                        len(format_record(displayed_records[current_index]))
                        - display_height,
                    ),
                )
                update_display()
            ...
            if query:
                ...
                search_results = [
                    ...
                    for r in records
                    if (
                        re.search(query, str(r.get("id", "")), re.IGNORECASE)
                        or re.search(query, r.get("source", ""), re.IGNORECASE)
                        or re.search(query, r.get("title", ""), re.IGNORECASE)
                        or re.search(query, r.get("summary", ""), re.IGNORECASE)
                        or re.search(query, r.get("text", ""), re.IGNORECASE)
                        or any(
