...
#            content += f"<tr><th>Text</th><td><p>{value.replace('\\n', '<br>')}</p></td></tr>"
...
def paginate_list(items, path_func, name_func, per_page=50):
    ...
    for i in range(total_pages):
        ...
        write_page(path_func(i), page_content)
...
paginate_list(sorted_titles_by_date, lambda p: f"titles_{p}.html", lambda p: f"All Titles (Page {p+1})")
...
for i in range(0, len(entities_sorted), PAGINATION_SIZE):
    ...
    if page_num == 0:
        write_page("entities.html", page_content)
    else:
        write_page(f"entities_{page_num}.html", page_content)

# Add pagination to main entities page
if len(entities_sorted) > PAGINATION_SIZE:
    page_links = render_pagination(0, (len(entities_sorted) + PAGINATION_SIZE - 1) // PAGINATION_SIZE,
                                   lambda p: f"entities_{p}.html" if p > 0 else "entities.html")
    write_page("entities.html", page_content + page_links)

# Entity Detail Pages
for entity, refs in entity_to_articles.items():
    titles_list = "".join([f"<li><a href='../articles/{idx}.html'>{title}</a> ({source})</li>" for idx, title, source in refs])
