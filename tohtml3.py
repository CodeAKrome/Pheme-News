...
for entity in sorted(entity_to_articles):
    ...
    entities_content += f"<li class='entity-item'><span class='entity-name'>{entity}</span><br>{titles_links}</li>"
entities_content += "</ul>"
...
for ent_type in sorted(type_to_entities):
    ents_content = f"<h1>Entities of Type: {ent_type}</h1><ul class='entity-detail'>"
    ...
    type_links += f"<li><a class='type-link' href='types/{ent_type}.html'>{ent_type}</a></li>"
...
print(f"✅ HTML files with styling, search, and enhanced structure generated in: {OUTPUT_DIR}")
