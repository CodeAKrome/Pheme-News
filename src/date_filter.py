...
def main():
    if len(sys.argv) != 2:
        ...
        print("    - Absolute date: yyyymmddThhmmss (e.g., 20250624T083154)", file=sys.stderr)
        print("    - Relative time: '-N unit' (e.g., '-1 day', '-2 weeks')", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        for input_data in sys.stdin:
            ...
            if 'published_parsed' not in data:
                print("Error: Input data missing 'published_parsed' field", file=sys.stderr)
                sys.exit(1)
