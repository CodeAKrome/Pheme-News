...
def extract_nodes_from_cypher(cypher_text):
    ...
    node_pattern = r'(?:CREATE|MERGE)\s*\(\s*(\w+)\s*(?::\s*(\w+))?\s*(?:\{([^}]+)\})?\s*\)'
    ...
    for match in matches:
        ...
        name = None
        if properties_str:
            name_match = re.search(r'name\s*:\s*["\']([^"\']+)["\']', properties_str, re.IGNORECASE)
            if name_match:
...
def find_similar_nodes(nodes, similarity_threshold=0.8):
    ...
    for i, node1 in enumerate(nodes):
        ...
        for j, node2 in enumerate(nodes[i+1:], i+1):
            ...
            if (similarity(node1['name'], node2['name']) >= similarity_threshold and
...
def remove_duplicates(cypher_text, similarity_threshold=0.8):
    ...
    for group in similar_groups:
        ...
        print(f"Found similar nodes (keeping '{keeper_node['name']}'):", file=sys.stderr)
...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Error: Invalid similarity threshold. Must be a number between 0 and 1", file=sys.stderr)
            sys.exit(1)
    else:
