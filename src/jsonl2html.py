    ...
    def format_value(self, value: Any, key: str = "") -> str:
        ...
        if value is None:
            ...
            if key == "tags" and all(isinstance(item, str) for item in value):
                tags_html = ''.join([f'<span class="tag">{escape(tag)}</span>' for tag in value])
            ...
            for i, item in enumerate(value):
                if isinstance(item, (dict, list)):
                    items.append(f'<div class="array-item"><strong>Item {i+1}:</strong><br>{self.format_value(item)}</div>')
                else:
                    items.append(f'<div class="array-item">{self.format_value(item)}</div>')
            return ''.join(items)
