...
if not URI or not PASS:
    print(
        "Error: NEO4J_URI and NEO4J_PASSWORD environment variables must be set.",
        file=sys.stderr,
    )
    sys.exit(1)
...
Examples:
  ...
  python %(prog)s bolt://localhost:7687 neo4j password < queries.cypher
    ...
    parser.add_argument(
        "--no-progress", "-n", action="store_true", help="Disable progress bar"
    )
        ...
        for line_num, query in queries:
            ...
            if records is None:  # Error occurred
                ...
                if progress_bar:
                    ...
                    print(
                        f"ERROR (line {line_num}): {summary_or_error}", file=sys.stderr
                    )
                ...
                if not args.continue_on_error:
                    if progress_bar:
                        progress_bar.close()
                    print(
                        "Stopping execution. Use --continue-on-error to continue on errors.",
                        file=sys.stderr,
                    )
                    break
            else:
                if BAR_RES:
                    ...
                    formatted_results = executor.format_results(
                        records, summary_or_error
                    )
                    if progress_bar:
