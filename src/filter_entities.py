        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if len(parts) == 2:
                    ...
                    print(f"Skipping malformed entity line: {line.strip()}", file=sys.stderr)
        return entities
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python filter_jsonl_by_entities.py <entity_file_path>", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Loaded {len(entities)} entities from {sys.argv[1]} for filtering", file=sys.stderr)
    ...
    print(f"Processed {total_records} records, matched {matched_records}", file=sys.stderr)
