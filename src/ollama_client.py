...
class OllamaClient:
            ...
            for attachment in attachments:
                try:
                    if self._is_image_file(attachment):
                        ...
                        logger.warning(f"Non-image attachment ignored: {attachment} (Ollama only supports images)")
    ...
    def copy_model(self, source: str, destination: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response.raise_for_status()
            return {'status': 'success', 'message': f'Model {source} copied to {destination}'}
        except requests.exceptions.RequestException as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not models:
            print("No models found. Please pull a model first (e.g., 'ollama pull llama2')")
            exit(1)
