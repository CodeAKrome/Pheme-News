...
class Decor():
    ...
    def log_error(self, error_message, log_file=None):
    ...
    def handler(self, e: Exception, func, msg: str = ""):
        """Exceptional dump"""
        ...
        if msg:
            self.log_error(f"Arrested: {msg} {preamble}")
        else:
            self.log_error(preamble)
        self.log_error("Traceback:")
        for frame in traceback.extract_tb(sys.exc_info()[2]):
            fname, lineno, fn, text = frame
            self.log_error(f"{lineno}:\t{fname}\n{fn}:\t{text})")


    def arrest(self, catch: list = [], msg: str = "", log_file=None):
    ...
    def test_self(self):
        ...
        @self.arrest([ValueError, TypeError], "Invalid argument")
