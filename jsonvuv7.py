...
class JsonViewer:
    ...
    def draw_summary(self):
        ...
        self.stdscr.addstr(max_y - 1, 0, "Right/Left: Change page")
    ...
    def draw_list(self):
        ...
        self.stdscr.addstr(max_y - 1, 0, "Up/Down: Navigate | Space: Select | Right/Left: Change page | " + status)
    ...
    def draw_edit(self):
        ...
        self.stdscr.addstr(max_y - 1, 0, "Up/Down: Navigate | Space: Remove | C: Clear All | Right/Left: Change page | " + status)
    ...
    def draw_export(self):
        ...
        if not self.current_export_record:
            ...
            return
        self.stdscr.addstr(row, 0, f"Exporting fields for record ID: {self.current_export_record['id']}")
        row += 1
        fields = sorted(self.current_export_record.keys())
        start = self.current_page * self.items_per_page
        end = min(start + self.items_per_page, len(fields))
        for idx, field in enumerate(fields[start:end], start):
            if row >= max_y - 2:
                break
            ...
            self.stdscr.addnstr(row, 0, item_str, max_x - 1, attrs)
            row += 1
        status = f"Page {self.current_page + 1}/{(len(fields) + self.items_per_page - 1)//self.items_per_page}"
        self.stdscr.addstr(max_y - 1, 0, "Up/Down: Select Field | Space: Toggle | Enter: Export | Esc: Cancel | Right/Left: Change page | " + status)
    ...
    def handle_input(self):
        try:
            ...
            if key == -1:
                return
            # Debug: Log key for verification (remove in production)
            # self.stdscr.addstr(1, 0, f"Key pressed: {key}  ")
    ...
    def handle_summary_input(self, key):
        ...
        if key == curses.KEY_LEFT and self.current_page > 0:
            self.current_page -= 1
            ...
            self.current_export_record = next((item for item in self.data if item['id'] in self.selected_ids), {})
    ...
    def handle_export_input(self, key):
        if not self.current_export_record:
            ...
            return
        fields = sorted(self.current_export_record.keys())
        max_pages = (len(fields) + self.items_per_page - 1) // self.items_per_page
        if key == curses.KEY_UP:
            self.cursor_pos = max(0, self.cursor_pos - 1)
        elif key == curses.KEY_DOWN:
            self.cursor_pos = min(self.items_per_page - 1, self.cursor_pos + 1)
