...
def main(stdscr, file_path):
    ...
    def get_displayed_records():
        nonlocal search_results, records, filter_ner_tag, filter_entity
        ...
        for r in candidates:
            ...
            if filter_entity:
                entity_filtered = any(
                    any(span["value"] == filter_entity for span in sent.get("spans", []))
                    for sent in r.get("ner", [])
                )

            if ner_filtered and entity_filtered:
                filtered.append(r)

        return filtered

    def show_entity_popup():
        """Show a popup listing all unique NER entities to select one."""
        entities = set()
        for record in records:
            for sentence_data in record.get("ner", []):
                for span in sentence_data.get("spans", []):
                    entities.add(span.get("value"))
        entity_list = sorted(entities)

        if not entity_list:
            return None

        popup_height = min(len(entity_list) + 2, max_y - 4)
        popup_width = min(40, max_x - 4)
        ...
        while True:
            ...
            for idx, entity in enumerate(entity_list[: popup_height - 2]):
                attr = curses.A_REVERSE if idx == choice else curses.A_NORMAL
                popup_win.addstr(idx + 1, 2, f" {entity} ", attr)
            ...
            if key == curses.KEY_UP and choice > 0:
                choice -= 1
            elif key == curses.KEY_DOWN and choice < len(entity_list) - 1:
                choice += 1
            elif key == 10:  # Enter
                return entity_list[choice]
    ...
    def update_status(message=None):
        ...
        parts = []
        if filter_ner_tag:
            parts.append(f"NER: {filter_ner_tag} | ")
        if filter_entity:
            parts.append(f"Entity: {filter_entity} | ")
        parts.append(f"{match_count} matches / {total_count} total | ")
        parts.append("↑↓: Sources | Left/Right: Navigate | PageUp/PageDown: Scroll | F: Toggle NER Filter | E: Entities | Esc: Clear Filters | q: Quit")
        text = "".join(parts)
    ...
    while True:
        ...
        if key == ord("q"):
            ...
            selected_entity = show_entity_popup()
            if selected_entity:
                filter_entity = selected_entity
                current_index = 0
                scroll_offset = 0
                update_display()
                update_status(f"Filtering by entity: {selected_entity}")
