...
def main(stdscr, file_path):
    ...
    def show_entity_popup():
        ...
        while True:
            ...
            if key == curses.KEY_UP:
                ...
                new_scroll = min(scroll_pos + visible_items, max(0, len(span_list) - visible_items))
                delta = new_scroll - scroll_pos
                scroll_pos = new_scroll
                choice = min(choice + delta, visible_items - 1)
                if scroll_pos + choice >= len(span_list):
                    choice = len(span_list) - scroll_pos - 1
                    if choice < 0:
                        choice = 0
    ...
    def update_status(message=None):
        ...
        parts.append("↑↓: Sources | ←→: Navigate | PageUp/Down: Scroll | F: Toggle NER Filter | E: Entities | Esc: Clear Filters | q: Quit")
        text = "".join(parts)
