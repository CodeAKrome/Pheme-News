...
def format_record(record, max_width=80):
    """Format the record as a list of lines, including NER spans."""
    lines = []
...
def main(stdscr):
    ...
    jsonl_file = Path("Pasted_Text_1745980366195.txt")  # Update path as needed
    ...
    def update_status(message=None):
        status_win.clear()
        text = message or (
            "Left/Right: Navigate | Space: Select | Enter: Search | Esc: Clear | q: Quit"
        )
