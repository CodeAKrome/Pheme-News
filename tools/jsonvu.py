...
def main(stdscr, file_path):
    ...
    # Precompute and cache all unique span texts from NER with their counts
    cached_span_texts = {}
    for record in records:
        for sentence_data in record.get("ner", []):
            for span in sentence_data.get("spans", []):
                span_text = span.get("text")
                if span_text:
                    if span_text not in cached_span_texts:
                        cached_span_texts[span_text] = 0
                    cached_span_texts[span_text] += 1
    span_list = sorted(cached_span_texts.keys())
    ...
    def show_entity_popup():
        """Show a scrollable popup listing all unique NER span.text values with counts and incremental search."""
        if not span_list:
            return None
        ...
        while True:
            ...
            for idx in range(min(visible_items, len(filtered_spans))):
                ...
                count = cached_span_texts[text]
                display_text = f"{text[:popup_width - 15]} ({count})"
                attr = curses.A_REVERSE if idx == choice else curses.A_NORMAL
                popup_win.addstr(idx + 1, 2, f" {display_text.ljust(popup_width - 6)} ", attr)
