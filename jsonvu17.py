...
def main(stdscr, file_path):
    ...
    def show_entity_popup():
        """Show a popup listing all unique NER entity texts to select one."""
        span_texts = set()
        for record in records:
            for sentence_data in record.get("ner", []):
                for span in sentence_data.get("spans", []):
                    span_texts.add(span.get("text"))
        span_list = sorted(span_texts)
        ...
        while True:
            ...
            for idx, text in enumerate(span_list[: popup_height - 2]):
                attr = curses.A_REVERSE if idx == choice else curses.A_NORMAL
                popup_win.addstr(idx + 1, 2, f" {text} ", attr)
            ...
            if key == curses.KEY_UP and choice > 0:
                choice -= 1
            elif key == curses.KEY_DOWN and choice < len(span_list) - 1:
                choice += 1
            elif key == 10:  # Enter
                return span_list[choice]
    ...
    def update_status(message=None):
        ...
        parts.append("↑↓: Sources | Left/Right: Navigate | PageUp/PageDown: Scroll | F: Toggle NER Filter | E: Select Entity Text | Esc: Clear Filters | q: Quit")
        text = "".join(parts)
    ...
    while True:
        ...
        if key == ord("q"):
            ...
            if sources:
                ...
                is_searching = True
                search_results = source_to_records[selected_source]
                search_results = [records[i] for i in search_results]
                current_index = 0
                scroll_offset = 0
                update_display()
                update_status()
